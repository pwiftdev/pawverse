...
// mintCharacter() stub. @solana/web3.js is dynamically imported so the game boots
...
export const wallet = {
  provider: null,
  address: null, // base58 string — the player's persistent character identity
...
export async function connectWallet() {
  ...
  if (!provider) {
    throw new Error(
      "No Solana wallet found — install Phantom or Backpack to use wallet identity.",
    );
  }
...
export async function checkPremium(address) {
  ...
  try {
    ...
    const accounts = await conn.getParsedTokenAccountsByOwner(
      ...
      { mint: new PublicKey(PREMIUM_MINT) },
    );
    ...
    console.warn(
      "[solana] premium check failed (treating as not-premium):",
      err.message,
    );
    return false;
  }
...
/**
 * Fetch the wallet's Pawverse NFTs to offer as selectable skins.
 *
 ...
 *   })});
 *   // then filter items whose metadata marks them as PAWVERSE characters and map
 *   // their attributes onto customization objects like below.
 */
export async function fetchCharacterNfts(address) {
  if (!address) return [];
  return [
    {
    ...
    {
      name: "Silver Bandit #009 (demo)",
      mock: true,
      customization: {
        breed: "silver-raccoon",
        primary: "#a8adb3",
        secondary: "#363b42",
        pattern: "mask",
        size: 1,
...
/**
 * mintCharacter() — devnet stub of the future character NFT flow.
 *
 * Today it builds a real transaction that embeds the character metadata in a Memo
 ...
 */
export async function mintCharacter(customization, name) {
  if (!wallet.provider || !wallet.address)
    throw new Error("Connect a wallet first.");
  if (NETWORK === "mainnet-beta") {
    // Deliberate guard: the stub is devnet-only until real minting lands.
    throw new Error(
      "mintCharacter() is a devnet stub — refusing to run on mainnet.",
    );
  }
