...
import { ACCEL, DECEL, GRAVITY, JUMP_VELOCITY, SPRINT_SPEED, SWIM_SPEED, WALK_SPEED, WORLD_BOUND } from './constants.js';
import { groundHeightAt, resolveObstacles, WATER_LEVEL } from './world.js';

export function createMoveState(x = 0, z = 12) {
  return { x, y: 0, z, vx: 0, vy: 0, vz: 0, yaw: 0, grounded: true, swimming: false };
}
 ...
 * @param speedMul breed speed multiplier
 */
export function stepMovement(s, input, dt, speedMul = 1) {
  ...
  const speed = (s.swimming ? SWIM_SPEED : input.sprint ? SPRINT_SPEED : WALK_SPEED) * speedMul;
  ...
  if (targetVx === 0 && targetVz === 0 && Math.hypot(s.vx, s.vz) < 0.05) { s.vx = 0; s.vz = 0; }
