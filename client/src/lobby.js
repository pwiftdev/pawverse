// ─── Lobby / character creator ───────────────────────────────────────────────
// Studio-style character creation: species tabs, preset cards with stat
...
import { makeCharacter } from "./characterfactory.js";
import { animateCharacter } from "./animator.js";
import {
  connectWallet,
  fetchCharacterNfts,
  mintCharacter,
...
const SPECIES_META = { dog: "Dogs", cat: "Cats", raccoon: "Raccoons" };
...
export function initLobby({ onPlay }) {
  ...
  let selectedSpecies = CHARACTER_MAP[state.custom.breed]?.species || "dog";
  ...
  function rebuildPreview() {
    ...
    view = makeCharacter({ ...state.custom, name: state.name || "preview" });
  ...
  (function loop() {
    ...
    if (view) {
      ...
      animateCharacter(view, state.pose, dt, POSE_SPEED[state.pose] || 0);
    }
  ...
  // ── species and character presets ────────────────────────────────────────
  const speciesRow = $("species-row");
  const breedGrid = $("breed-grid");
  function applyPreset(preset) {
    state.custom.breed = preset.id;
    state.custom.primary = preset.primary;
    state.custom.secondary = preset.secondary;
    state.custom.pattern = preset.pattern;
    syncAll();
    save();
  }
  ...
  function renderSpecies() {
    ...
    for (const species of SPECIES) {
      ...
      el.className = `species-btn${selectedSpecies === species ? " sel" : ""}`;
      el.textContent = SPECIES_META[species];
      el.onclick = () => {
        selectedSpecies = species;
        const current = CHARACTER_MAP[state.custom.breed];
        ...
        const next = CHARACTER_PRESETS.find(
          (preset) =>
            preset.species === species && (!preset.premium || premiumUnlocked),
        );
  ...
  function renderBreeds() {
    breedGrid.innerHTML = "";
    for (const b of CHARACTER_PRESETS.filter(
      (preset) => preset.species === selectedSpecies,
    )) {
      ...
      el.onclick = () => {
        if (locked) {
          setWalletStatus(
            `💎 ${b.name} is premium — hold the configured token to unlock it (see the Wallet tab).`,
          );
          return;
        }
        applyPreset(b);
  ...
  function renderStyleControls() {
    renderPalette(
      "pal-primary",
      () => state.custom.primary,
      (v) => {
        state.custom.primary = v;
      },
      COAT_COLORS,
    );
    renderPalette(
      "pal-secondary",
      () => state.custom.secondary,
      (v) => {
        state.custom.secondary = v;
      },
      COAT_COLORS,
    );
  ...
  $("randomize").onclick = () => {
    const pool = CHARACTER_PRESETS.filter(
      (preset) =>
        preset.species === selectedSpecies &&
        (!preset.premium || premiumUnlocked),
    );
    ...
    state.custom = {
      ...
      collar:
        Math.random() < 0.8
          ? COLLAR_COLORS[Math.floor(Math.random() * COLLAR_COLORS.length)]
          : null,
  ...
  walletBtn.onclick = async () => {
    ...
    try {
      ...
      setWalletStatus(
        `connected on ${solanaConfig.NETWORK} — character identity tied to ${short}` +
          (solanaConfig.PREMIUM_MINT
            ? wallet.premium
              ? " · premium unlocked"
              : " · premium token not held"
            : " · all characters unlocked (no premium mint configured)"),
      );
      ...
      syncAll();
      renderNftSkins(await fetchCharacterNfts(wallet.address));
    } catch (err) {
  ...
  mintBtn.onclick = async () => {
    ...
    try {
      const sig = await mintCharacter(state.custom, state.name || "Paw");
      setWalletStatus(`minted (stub memo tx): ${sig.slice(0, 10)}…`);
    } catch (err) {
      setWalletStatus(`⚠ mint: ${err.message}`);
    }
    mintBtn.disabled = false;
    mintBtn.textContent = "⛏ mint this character (devnet stub)";
  };
  ...
  function renderNftSkins(nfts) {
    ...
    if (!nfts.length) {
      root.innerHTML =
        '<span style="font-size:12px;color:var(--dim)">no character NFTs in this wallet</span>';
      return;
    }
  ...
  // ── sync + persistence ───────────────────────────────────────────────────
  function syncAll() {
    selectedSpecies = CHARACTER_MAP[state.custom.breed]?.species || "dog";
    renderSpecies();
    renderBreeds();
    renderStyleControls();
    renderExtras();
  ...
  function save() {
    ...
    if (wallet.address)
      localStorage.setItem(`${LS_KEY}.${wallet.address}`, json);
  }
  function loadLoadout(address) {
    try {
      const raw = localStorage.getItem(
        address ? `${LS_KEY}.${address}` : LS_KEY,
      );
  ...
  $("play").onclick = () => {
    ...
    const preset = CHARACTER_MAP[state.custom.breed];
    if (preset?.premium && !premiumUnlocked) state.custom.breed = "shiba";
    save();
    $("lobby").style.display = "none";
    onPlay(state.name.trim() || "Paw", { ...state.custom });
  };
...
function clampPct(v) {
  return Math.min(100, Math.max(8, v));
}
