...
function env(ctx, node, points) {   // [[t, gain], …]
  const g = ctx.createGain();
  g.gain.setValueAtTime(points[0][1], points[0][0]);
  for (const [t, v] of points.slice(1)) g.gain.exponentialRampToValueAtTime(Math.max(0.0001, v), t);
  node.connect(g);
...
const synthBark = () => render(0.3, (ctx) => {
  ...
  env(ctx, sub, [[0, 0.0001], [0.012, 0.75], [0.2, 0.0001]]);
  ...
  env(ctx, formant, [[0, 0.0001], [0.018, 1.0], [0.12, 0.5], [0.26, 0.0001]]);
  ...
  breath.connect(bp);
  env(ctx, bp, [[0, 0.0001], [0.008, 0.4], [0.11, 0.0001]]);

  sub.start(); voice.start(); breath.start();
});

const synthThud = () => render(0.16, (ctx) => {       // landing after a jump
  const osc = ctx.createOscillator();
  osc.type = 'sine';
  osc.frequency.setValueAtTime(110, 0);
  osc.frequency.exponentialRampToValueAtTime(45, 0.13);
  env(ctx, osc, [[0, 0.0001], [0.008, 0.6], [0.14, 0.0001]]);
  const noise = ctx.createBufferSource();
  noise.buffer = noiseBuffer(ctx, 0.05);
  const lp = ctx.createBiquadFilter();
  lp.type = 'lowpass'; lp.frequency.value = 700;
  noise.connect(lp);
  env(ctx, lp, [[0, 0.0001], [0.006, 0.25], [0.045, 0.0001]]);
  osc.start(); noise.start();
});

const synthYelp = () => render(0.3, (ctx) => {
  const osc = ctx.createOscillator();
  osc.type = 'triangle';
  ...
  env(ctx, osc, [[0, 0.0001], [0.02, 0.7], [0.28, 0.0001]]);
  osc.start();
});

const synthHowl = () => render(1.8, (ctx) => {
  const osc = ctx.createOscillator();
  osc.type = 'sine';
  ...
  env(ctx, osc, [[0, 0.0001], [0.15, 0.55], [1.3, 0.4], [1.75, 0.0001]]);
  osc.start(); vib.start();
});

const synthSplash = () => render(0.5, (ctx) => {
  const noise = ctx.createBufferSource();
  ...
  noise.connect(lp);
  env(ctx, lp, [[0, 0.0001], [0.02, 0.8], [0.45, 0.0001]]);
  noise.start();
});

const synthPop = () => render(0.12, (ctx) => {   // ball pickup
  const osc = ctx.createOscillator();
  osc.type = 'square';
  osc.frequency.setValueAtTime(500, 0);
  osc.frequency.exponentialRampToValueAtTime(900, 0.08);
  env(ctx, osc, [[0, 0.0001], [0.01, 0.4], [0.1, 0.0001]]);
  osc.start();
});

const synthWhoosh = () => render(0.35, (ctx) => { // ball throw
  const noise = ctx.createBufferSource();
  ...
  noise.connect(bp);
  env(ctx, bp, [[0, 0.0001], [0.05, 0.5], [0.32, 0.0001]]);
  noise.start();
});

const synthChime = () => render(0.7, (ctx) => {   // pet/feed reward
  for (const [f, t0] of [[660, 0], [880, 0.12], [1100, 0.24]]) {
    const osc = ctx.createOscillator();
    osc.type = 'sine'; osc.frequency.value = f;
    env(ctx, osc, [[t0, 0.0001], [t0 + 0.02, 0.3], [t0 + 0.4, 0.0001]]);
    osc.start(t0);
  }
});

const synthGrowl = () => render(0.4, (ctx) => {   // play-bite tussle
  const osc = ctx.createOscillator();
  osc.type = 'sawtooth';
  ...
  trem.connect(tremGain); tremGain.connect(osc.frequency);
  env(ctx, osc, [[0, 0.0001], [0.03, 0.5], [0.38, 0.0001]]);
...
export class GameAudio {
  ...
  async init(scene) {
    ...
    const [bark, yelp, howl, splash, pop, whoosh, chime, growl, thud] = await Promise.all([
      synthBark(), synthYelp(), synthHowl(), synthSplash(), synthPop(), synthWhoosh(),
      synthChime(), synthGrowl(), synthThud(),
    ]);
    this.buffers = { bark, yelp, howl, splash, pop, whoosh, chime, growl, thud };
    this.ready = true;
    this.nightK = 0;
    this.startAmbient();
  }
  ...
  startAmbient() {
    ...
    noise.connect(lp); lp.connect(this.seaGain);
    ...
    const wildlife = () => {
      ...
      const g = ctx.createGain();
      g.gain.value = 0;
      osc.connect(g); g.connect(this.listener.getInput());
      if (this.nightK > 0.5) {
        ...
        for (let n = 0; n < notes; n++) {
          ...
          osc.frequency.exponentialRampToValueAtTime(base * (1.2 + Math.random() * 0.3), t + 0.07);
          g.gain.setValueAtTime(0, t);
